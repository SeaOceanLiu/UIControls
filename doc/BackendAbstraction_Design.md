...
#### 13.5 Cursor 回调表工厂（Phase 16h）

2026-07-12：`UIBackendCallbacks` 结构体新增三个函数指针，支持 fromsource/DLL 桥接模式下的光标工厂注册：
...
**问题**：raylib fromsource 测试（`test_fromsource_cabi` raylib 后端）中所有纹理不可见。`DrawTexturePro` 从 `UICornerstone.dll` 经回调桥接调用到 exe 中的 `RaylibRenderDevice::drawTexture`，日志显示正确的纹理 ID 和 dst rect，但画面上无任何纹理。
