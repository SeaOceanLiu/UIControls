...
布局配置文件 (layouts/*.json)
   ...
   LayoutParser (解析器)
    ...
    │        └─ parseWinFrame   → make_shared<WinFrame>
    ├─ 属性解析工具集
...
| WinFrame    | 可拖拽/缩放窗口控件，含标题栏/关闭按钮/客户区域 ✅        |
...
### Phase 5 — ComboBox + 焦点系统


| 功能               | 说明                                          | 状态 |
| ------------------ | --------------------------------------------- | ---- |
| ComboBox           | 下拉选择框控件，支持列表/滚轮/键盘/搜索       | ✅   |
| Popup 焦点边界     | Popup 内 Tab 循环，不越界                    | ✅   |
| ComboBox JSON      | LayoutParser 解析 `items/selectedIndex/placeholder` 等 | ✅   |
| ComboBox C ABI     | 6 个函数：`CreateComboBox/SetItems/SetSelectedIndex/GetSelectedIndex/GetSelectedLabel/SetOnSelectionChanged` | ✅   |
| C ABI 鼠标滚轮坐标 | 桥接事件增加 x/y 坐标传递                     | ✅   |
...
TextArea 继承自 EditBox，支持所有 EditBox 属性，并额外  支持多行文本布局：
...
WinFrame 是一个可拖拽、可缩放（可选）的窗口控件，内部包含标题栏、标题文字、关闭按钮和客户区域 Panel：
...
```jsonc
{
    ...
    "title": "窗口标题",                              // 标题栏文字
    "edgeMargin": 6.0,                                // 边缘缩放热区宽度（可选，默认 4.0）
    "resizable": true,                                // 是否允许缩放（可选，默认 true）
    "colors": {
        "background": { "normal": "#303030FF" },      // 整体背景色
        "border": { "normal": "#606060FF" },          // 边框色
        "titleBar": {
            "bg": { "normal": "#2A2A50FF" }           // 标题栏背景色
        },
...
- WinFrame 创建后自动 `hide()`，需通过代码 `show()` 显示
- 子控件通过 `"children"` 属性自动添加到客户区域（ClientPanel）
- `"resizable": false` 时边缘热区不响应缩放，光标保持缺省样式
- `"edgeMargin"` 控制边缘热区宽度（local 单位，默认 4.0px）
- 颜色通过 `"colors"` 对象的多个子字段分别控制不同区域的独立颜色
...
```jsonc
{
    ...
    "swatchSize": 16.0,                                 // 关闭状态色块尺寸（float）
    "closedFontSize": 14,                               // 关闭状态 hex 文本字号
    "closedTextColor": "#FFFFFFFF",                     // 关闭状态 hex 文本颜色
    "popupBGColor": "#303030FF",                        // 弹出面板背景色
    "events": {
        "onColorChanged": "handleColorChanged"          // 颜色变更事件
    }
}
```
...
| 字段               | 类型   | 必填 | 默认值            | 说明                     |
| ------------------ | ------ | ---- | ----------------- | ------------------------ |
| `color`            | string | 否   | `"#FFFFFFFF"`     | 初始颜色 hex 字符串      |
| `presets`          | array  | 否   | `[]`              | 预设色块 hex 字符串数组  |
| `presetLayout`     | object | 否   | `{"cols":8,"rows":2}` | 预设面板网格行列数    |
| `swatchSize`       | number | 否   | `16.0`            | 关闭状态色块尺寸         |
| `closedFontSize`   | int    | 否   | `14`              | 关闭状态 hex 文本字号    |
| `closedTextColor`  | string | 否   | `"#FFFFFFFF"`     | 关闭状态 hex 文本颜色    |
| `popupBGColor`     | string | 否   | `"#303030FF"`     | 弹出面板背景色           |

**Standard 通用属性**：`scale`, `visible`, `enabled`, `colors.background`/`border`/`text` 均受支持。
...
```jsonc
{
    ...
    "arrowWidth": 24.0,                                        // 箭头按钮宽度
    "itemHeight": 24.0,                                        // 下拉列表项高度
    "maxVisibleItems": 6,                                      // 最大可见项数（超出滚动）
    "cycleEnabled": true,                                      // 上下箭头是否循环选中
    "events": {
        "onSelectionChanged": "handleCitySelected"             // 选中变化事件
    }
}
```
...
| 字段               | 类型    | 必填 | 默认值   | 说明                     |
| ------------------ | ------- | ---- | -------- | ------------------------ |
| `items`            | array   | 否   | `[]`     | 下拉列表项字符串数组     |
| `selectedIndex`    | int     | 否   | `-1`     | 初始选中项索引（-1=无）  |
| `placeholder`      | string  | 否   | `""`     | 占位文本                 |
| `arrowWidth`       | number  | 否   | `20.0`   | 箭头按钮宽度             |
| `itemHeight`       | number  | 否   | `22.0`   | 下拉列表项高度           |
| `maxVisibleItems`  | int     | 否   | `8`      | 最大可见项数             |
| `cycleEnabled`     | bool    | 否   | `false`  | 上下箭头是否循环选中     |
...
见 `layouts/test_layout.json`（第 10 节）。
...
| ComboBox    | `onSelectionChanged` | `setOnSelectionChanged()` | `void(shared_ptr<ComboBox>, int)`                        |
...
parseControl(j, parent, index)
  ...
  │     ├─ "ComboBox"    → parseComboBox(j, parent)
  │     ├─ "Slider"      → parseSlider(j, parent)
  │     ├─ "ColorPicker" → parseColorPicker(j, parent)
  │     ├─ "Panel"       → parsePanel(j, parent)
  │     ├─ "WinFrame"    → parseWinFrame(j, parent)
...
```
parseWinFrame(j, parent)
  │
  ├─ 1. 解析 rect + scale → 创建 WinFrame
  │
  ├─ 2. parseCommonProperties
  │
  ├─ 3. 解析 WinFrame 特有属性
  │     ├─ j["title"]          → setTitle
  │     ├─ j["edgeMargin"]     → setEdgeMargin
  │     └─ j["resizable"]      → setResizable
  │
  ├─ 4. 解析颜色
  │     ├─ colors["background"]  → setBackgroundStateColor
  │     ├─ colors["border"]      → setBorderStateColor
  │     ├─ colors["titleBar"]    → 传递给 m_titleBar
  │     └─ colors["titleText"]   → 传递给 m_titleLabel
  │
  ├─ 5. 子控件 → parseChildren(ClientPanel, j)
  │
  ├─ 6. parseEvents（onClose 事件）
  │
  ├─ 7. 注册 ID
  │
  ├─ 8. winFrame->create()
...
void LayoutParser::parseEvents(shared_ptr<ControlImpl> ctrl, const json& j) {
    ...
    if (auto cb = dynamic_pointer_cast<ComboBox>(ctrl)) {
        if (events.contains("onSelectionChanged") && events["onSelectionChanged"].is_string()) {
            ...
            auto it = m_handlers.find(handlerName);
            if (it != m_handlers.end()) {
                cb->setOnSelectionChanged([this, handlerName](shared_ptr<ComboBox> sender, int) {
                    m_handlers[handlerName](sender);
                });
            }
        }
