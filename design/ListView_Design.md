...
> 修订注记：v9（2026-08-17）——**属性一致性四层同步（§5.6 矩阵）**：核心属性（mode/multiSelect/selectedIndex/cycleNavigation/rowHeight/headerHeight/gridlines/horizontalGridlines/hover/minColumnWidth/sortColumn）C++/JSON/CABI/Binding **四层全量一期**；`cycleNavigation`/`selectedIndex`/`multiSelect`/`sortColumn`/`sortAscending` 为新增（键名仿 TreeView `PropertyNames.h:127/596`）；`headerStyle`/`cellStyle` JSON 与 Binding 二期（C++/CABI 一期）显式标注；`setColumnSorter` 运行时注入 JSON 无字段显式标注；§5.0.3 补"── 属性 ──"小节；§5.4 补循环/多选开关/初始选中
> 修订注记：v10（2026-08-17）——**API 精简 + 命名规范核对**：① 属性类**走通用接口**——CABI 通用 setter（`SetBool/SetFloat/SetInt/SetEnum`，kebab-case 属性名惯例 `"row-height"`/`"selected-index"` 等，先例 UICornerstoneAPI 属性系统通用入口），删除专用 `ListViewSetMode`/`ListViewSetSelectedRow`/`ListViewGetSelectedRow`/`ListViewSetSort`；② C++ Binding **统一属性接口** `setProperty(key, value)`（键名 = JSON camelCase 同串），不建 11 个独立属性 setter；③ 命名规范：C++ `set+UpperCamel` / JSON camelCase / CABI 通用属性 kebab-case / CABI 专用函数 `ListView+Verb+Object`；④ `Mode` 枚举定义（`Mode::Multi/Single`）；⑤ `setSortColumn(int)`+`setSortAscending(bool)` 两独立函数（对齐 JSON 双字段）
...
| 树形列表 | TreeView：树结构 + 单选（`m_selectedId`/`m_selectedRow`，`TreeView.h:82-84`）+ leadingControl 增强（已实施）+ JSON 全链（`"tree-view"`，`PropertyNames.h:656`） |
...
> **分层命名惯例**：C++ = `set+UpperCamel`；JSON = camelCase（既有键先例 `PropertyNames.h:127/509/596`）；CABI 通用属性 = **kebab-case**（既有惯例："row-height"/"indent-width"/"font-size"，`UICornerstoneAPI.h:449-470`）；C++ Binding 属性键 = JSON 键（camelCase）同串。
...
| 初始选中 `selectedIndex` | `setSelectedRow` | `"selectedIndex"`（键名同 TreeView，`PropertyNames.h:127`） | `SetInt(inst, lv, "selected-index", v)` / `GetInt` 读 | `setProperty("selectedIndex", v)` |
...
- **可配属性字段**（一期）：`mode`/`multiSelect`/`selectedIndex`/`cycleNavigation`/`rowHeight`/`headerHeight`/`gridlines`/`horizontalGridlines`/`hover`/`minColumnWidth`/`sortColumn`/`sortAscending`（键名 camelCase，仿 TreeView 惯例 `PropertyNames.h:509/596`；**四层一致性见 §5.6 矩阵**）
...
- **CABI**（一期，**属性走通用 setter，专用函数仅数据/列表/对象类**——见 §5.6 矩阵）：
  - 通用属性：`SetEnum(inst, lv, "mode", ...)`、`SetBool(inst, lv, "multi-select"/"hover"/"gridlines"/"horizontal-gridlines"/"cycle-navigation"/"sort-ascending", v)`、`SetFloat(inst, lv, "row-height"/"header-height"/"min-column-width", v)`、`SetInt(inst, lv, "selected-index"/"sort-column", v)` + `GetInt(inst, lv, "selected-index", &out)` 读选中（kebab-case 惯例）
...
| 文件 | 改动 |
|---|---|
| `include/ListView.h`（新） | ListView + ListColumn/ListRow + Builder；ControlType 枚举加 ListView（`ControlBase.h:139`） |
...
| `include/PropertyNames.h` | 键注册：kControlTypeListView="list-view" + 属性键 mode/multi-select/header-height/gridlines/horizontal-gridlines/hover/min-column-width/sort-column/sort-ascending（selected-index/cycle-navigation/row-height 复用既有键）+ JSON camelCase 键 |
| `docs/schema/declarative-ui.schema.json` | $defs/list-view（columns/rows/cellControls 结构，properties 字母序）+ control-any oneOf 追加 |
| `layouts/all_controls.json` | 追加 list-view 样例（multi 三列排序 + single 单列） |
| AGENTS.md 验收链 | README/用户手册/API_Mapping_Table 同步（验收清单 11/12/14 条） |

## 9.1 测试策略

1. **单元断言**（test_listview.cpp）：数据模型回环（addRow/insertRow/removeRow/getColumnValues 对齐语义/写 API 补足——含 §5.0.3 反例：`["C"]` 落 A 列）；单列模式视觉等同 ListBox；选择模型（单选/Ctrl 多选/setSelectedRow）；排序（字典序缺省 + 自定义回调 + 稳定排序 + 排序后选中按 id 跟随）；四层属性回环抽查（mode/selectedIndex/rowHeight 等）
2. **可视化矩阵**：multi 模式三列表（列头/排序箭头/网格线/选中/hover/单元格控件）+ single 模式（ListBox 视觉）
3. **键盘**：↑↓ 移动/Home/End/循环开关（模拟 KeyDown 断言）
4. **截图**：帧内 CaptureViewport 存 Temp/listview_capture.bmp，SDL3 优先逐像素核对
5. **三后端回归**：test_listview + 存量测试无破坏 + layouts --strict
...
- 一期交付 = 本文档全部拍板内容；headerStyle/cellStyle 的 JSON 与 Binding 二期（C++/CABI 一期先行，§5.6 矩阵已标注）
- Icon/SmallIcon 模式、Shift 键盘多选、数值排序识别、奇偶条纹、十万行分页：不做（§10/§11）
- CABI 专用函数按 §7 清单实施；属性类一律通用 setter（不建专用）
